    ...
    Context 'New-ColorScriptCache skipped and error paths' {
        ...
        It 'skips by user when ShouldProcess declines for a targeted name' {
            InModuleScope ColorScripts-Enhanced {
                ...
                $res = New-ColorScriptCache -Name 'Galaxy' -PassThru -Force
                $res | Should -Not -BeNullOrEmpty
                ($res | Select-Object -First 1).Status | Should -Be 'SkippedByUser'
            }
        }

        It 'skips up-to-date and returns cached content when PassThru is requested' {
            InModuleScope ColorScripts-Enhanced -Parameters @{ root = $script:CacheRoot } {
                ...
                $cachePath = Join-Path $root 'Galaxy.cache'
                ...
                $galaxyScript = Join-Path -Path (Join-Path -Path $script:ModuleRoot -ChildPath 'Scripts') -ChildPath 'Galaxy.ps1'
                $signature = Get-FileContentSignature -Path $galaxyScript -IncludeHash
                Write-CacheEntryMetadataFile -ScriptName 'Galaxy' -Signature $signature -CacheFile $cachePath
                $res = New-ColorScriptCache -Name 'Galaxy' -PassThru
                $rec = $res | Where-Object Name -CEQ 'Galaxy' | Select-Object -First 1
                $rec.Status | Should -Be 'SkippedUpToDate'
                $rec.StdOut.Trim() | Should -BeExactly 'precache'
            }
        }
    }

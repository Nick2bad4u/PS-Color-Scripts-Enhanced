...
for ($y = 0; $y -lt $rows; $y++) {
    ...
    for ($x = 0; $x -lt $cols; $x++) {
        ...
        $rings = [math]::Floor($distance / 3)
        $hue = ($angle / (2 * [math]::PI) + $rings * 0.1) % 1
        $sat = 0.9 - ($distance / [math]::Max($centerX, $centerY)) * 0.5
        $val = 0.6 + 0.4 * [math]::Sin($distance * 0.5 + $angle * 2)
        $rgb = Convert-HsvToRgb -Hue $hue -Saturation ([math]::Min([math]::Max($sat, 0.0), 1.0)) -Value ([math]::Min([math]::Max($val, 0.0), 1.0))
        $petals = [math]::Floor([math]::Abs([math]::Sin($angle * 6)) * 4)
        $symbols = @('✦', '✧', '✩', '✪', '✫', '✬', '✭', '✮')
        $symbol = $symbols[$petals % $symbols.Length]

...
# IMPORTANT:
# The test/build pipeline calls functions that clear and rebuild caches.
# Never point tests at the user's real %APPDATA%\ColorScripts-Enhanced cache.
$testRunRoot = Join-Path -Path ([System.IO.Path]::GetTempPath()) -ChildPath ('ColorScripts-Enhanced-TestRun-' + [guid]::NewGuid().ToString('N'))
...
if (-not $SkipPester) {
    Write-Host "\n=== Pester Test Suite ===" -ForegroundColor Cyan
    try {
        $repoRoot = Split-Path -Parent $PSScriptRoot
        $testsRoot = Join-Path $repoRoot 'Tests'
        $pesterResult = Invoke-Pester -Path $testsRoot -Output Minimal -PassThru
        if ($pesterResult) {
            $SuccessCount += [int]$pesterResult.PassedCount
            $ErrorCount += [int]$pesterResult.FailedCount
        }
    }
    catch {
        $ErrorCount++
        Write-Host "✗ Pester run failed: $($_.Exception.Message)" -ForegroundColor Red
    }
}

if (-not $SkipCustomTests) {
    # Test 1: Module loaded
    Test-Function 'Module imports successfully' {
        # IMPORTANT:
        # This custom test script intentionally creates and clears caches.
        # Force an isolated cache/config root so local builds/tests do not touch the user's real
        # %APPDATA%\ColorScripts-Enhanced directory.
        ...
        function Invoke-TestRunCleanup {
            ...
            try {
                ...
                Write-Verbose ("Failed to remove test temp directory '{0}': {1}" -f $testRunRoot, $_.Exception.Message)
            }
        }

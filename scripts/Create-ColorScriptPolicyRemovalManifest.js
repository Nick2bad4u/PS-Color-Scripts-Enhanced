...
const { isSourceFidelityLocked } = require("./Audit-ColorScriptContent.js");
...
function createPolicyRemovalManifest(
    ...
    const works = selectedIds
        .map((id) => {
            ...
            const scripts = source.scripts.map((name) => {
                ...
                const scriptPath = path.join(scriptsDirectory, `${name}.ps1`);
                ...
                if (
                    isSourceFidelityLocked(fs.readFileSync(scriptPath, "utf8"))
                ) {
                    throw new Error(
    ...
    return {
        ...
        policy: "Remove works confirmed by official archive tags and manual preview review to contain explicit nudity or adult content. Ambiguous, clothed, or merely suggestive works are not selected automatically.",
        summary: {
    ...
    for (const argument of arguments_) {
        if (argument.startsWith("--audit=")) {
            options.auditPaths.push(
                path.resolve(REPOSITORY_ROOT, argument.slice("--audit=".length))
            );
        } else if (argument.startsWith("--include=")) {
            const id = argument.slice("--include=".length);
            if (!/^16colors:[^/]+\/[^/]+$/u.test(id)) {
                throw new Error(`Invalid 16colors source identifier: ${id}`);
            }

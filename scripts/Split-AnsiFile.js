...
function extractLinesFromPs1(filePath) {
  ...
  if (!match) {
    throw new Error(`Unable to locate Write-Host here-string in ${filePath}.`);
  }
...
function main(argv = process.argv.slice(2)) {
  ...
  if (positional.length === 0) {
    ...
    console.error("  --format=ps1|ansi          Output format (default: ps1)");
    console.error(
    ...
    console.error("  --heights=h1,h2,...        Segment heights (cumulative)");
    console.error("  --every=<n>                Split after every <n> lines");
    console.error(

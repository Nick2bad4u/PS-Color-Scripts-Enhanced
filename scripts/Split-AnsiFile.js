...
function extractLinesFromPs1(filePath) {
    ...
    if (safeHereStringMatch) {
        return safeHereStringMatch[1].replace(/\r\n?/g, "\n").split("\n");
    }
...
function chooseBalancedBreaks(
    ...
    /**
     * Prefer larger earlier segments when two complete plans have exactly the
     * same score. This matches the conventional even split where remainder rows
     * are assigned from the first part onward.
     *
...
function ensureTrailingReset(content) {
    ...
    const trailingLineBreaks = content.match(/(?:\r?\n)+$/u)?.[0] || "";
    const body = trailingLineBreaks
        ? content.slice(0, -trailingLineBreaks.length)
        : content;

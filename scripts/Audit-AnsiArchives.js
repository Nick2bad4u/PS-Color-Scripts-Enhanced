...
function createSixteenColorsCandidate(pack, filename, metadata, declaredSize) {
    const listedPack = pack._listedPack
        ...
        : null;
    const archive =
        typeof pack.archive === "string"
            ? pack.archive
            : typeof listedPack?.archive === "string"
              ? listedPack.archive
              : null;
    ...
    return {
        ...
        archiveUrl:
            typeof pack.download === "string"
                ...
                : typeof listedPack?.download === "string"
                  ? new URL(listedPack.download, SIXTEEN_COLORS_SITE).href
                  : null,
        filename,
        ...
        const flushText = () => {
            ...
            if (/\S/u.test(value)) {
                const decorations = [
                    ...
                    .join(" ");
                text.push(
                    `<text x="${textRun.start * cellWidth}" y="${rowNumber * cellHeight + 13}" fill="${textRun.color}" font-family="Consolas, 'Courier New', monospace" font-size="16"${textRun.bold ? ' font-weight="700"' : ""}${decorations ? ` text-decoration="${decorations}"` : ""} textLength="${textRun.chars.length * cellWidth}" lengthAdjust="spacingAndGlyphs" xml:space="preserve">${escapeXmlText(value)}</text>`
                );
            }
            textRun = null;
        };
...
function classifyCandidate(candidate, existingHashes) {
    ...
    return {
        ...candidate,
        disposition: likelyFiller
            ? "pending-review-filler"
            : width > 120
              ? "pending-review-wide"
              : height > 50

...
function colorToCodes(color, isForeground) {
    ...
    switch (color.mode) {
        ...
        case 'bright':
            return typeof color.value === 'number' ? [brightBase + color.value] : [];
        case 'palette':
            return typeof color.value === 'number'
                ? [isForeground ? 38 : 48, 5, color.value]
                : [];
        case 'rgb':
            if (
...
function parseSauceRecord(buffer) {
    return {
        ...
        title: buffer.subarray(7, 42).toString('ascii').replace(/\0+$/, '').trim(),
        author: buffer.subarray(42, 62).toString('ascii').replace(/\0+$/, '').trim(),
        group: buffer.subarray(62, 82).toString('ascii').replace(/\0+$/, '').trim(),
...
function stripSauce(buffer) {
    ...
    const sauceId = buffer.subarray(sauceOffset, sauceOffset + 5).toString('ascii');
    if (sauceId !== 'SAUCE') {
        return { buffer, sauce: null };
    }

    const sauceRecord = buffer.subarray(sauceOffset, sauceOffset + SAUCE_LENGTH);
    ...
    if (sauce.comments > 0) {
        ...
        if (commentOffset >= 0) {
            const commentId = buffer.subarray(commentOffset, commentOffset + 5).toString('ascii');
            if (commentId === 'COMNT') {
...
class TerminalEmulator {
    ...
    constructor(options = {}) {
        const opts = options || {};
        this.columns = typeof opts.columns === 'number' && opts.columns > 0 ? opts.columns : DEFAULT_COLUMNS;
    ...
    eraseInDisplay(mode) {
        ...
        if (mode === 0) {
            this.eraseInLine(0);
            const keys = [...this.rows.keys()].filter((row) => row > this.cursorY);
            ...
            switch (code) {
                ...
                case 38: {
                    ...
                    if (next === 5 && values[i + 2] !== undefined) {
                        this.currentAttrs.fg = { mode: 'palette', value: values[i + 2] };
                        i += 2;
                ...
                case 48: {
                    const next = values[i + 1];
                    if (next === 5 && values[i + 2] !== undefined) {
                        this.currentAttrs.bg = { mode: 'palette', value: values[i + 2] };
                        i += 2;
                    } else if (next === 2 && values[i + 4] !== undefined) {
                ...
                default:
                    if (code >= 30 && code <= 37) {
                        ...
                        this.currentAttrs.bg = { mode: 'basic', value: code - 40 };
    ...
    applyCsi(collected, params, flag) {
        ...
        switch (flag) {
            ...
            case 'u':
                ...
                break;
            default:
                this.warnings.push({ type: 'CSI', collected, params: [...values], flag });
                break;
        }
    }
        ...
        for (let rowIndex = 0; rowIndex <= this.maxRow; rowIndex += 1) {
            ...
            for (let col = 0; col <= row.maxCol; col += 1) {
                ...
                if (cell) {
                    ...
                    if (this.stripSpaceBackground && cell.char === ' ' && cell.attrs.bg) {
            ...
            while (effectiveLength > 0) {
                ...
                if (lastCell.char === ' ' && attrsEqual(lastCell.attrs, defaultAttrs)) {
                    effectiveLength -= 1;
                } else {
                    break;
                }
...
function parseArguments(argv) {
    const options = /** @type {{columns: number | null, autoWrap: boolean, stripSpaceBackground: boolean}} */ ({
        columns: null,
        autoWrap: true,
    ...
    for (const arg of argv) {
        ...
        } else if (arg === '--strip-space-bg' || arg === '--strip-space-background') {
 ...
 * @returns {{lines: string[], warnings: Array<Record<string, unknown>>, terminal: TerminalEmulator}}
 */
...
function main(argv = process.argv.slice(2)) {
    ...
    if (positional.length === 0) {
        console.error('Usage: node Convert-AnsiToColorScript.js [options] <ansi-file> [output-file]');
        console.error('Options:');
        console.error('  --columns=<n>      Set terminal column width (defaults to SAUCE width or 80).');
    console.error('  --no-autowrap      Disable automatic line wrapping at the terminal width.');
    console.error('  --strip-space-bg   Clear background color for plain space characters.');
        process.exit(1);
    }
    ...
    try {
        ...
        const terminalColumns = options.columns || (sauce && sauce.tInfo1 ? sauce.tInfo1 : DEFAULT_COLUMNS);
        ...
        console.log(`Using terminal width: ${terminalOptions.autoWrap ? terminalColumns : 'no wrap'}`);
        ...
        const { lines, warnings, terminal } = convertAnsiToPs1(content, terminalOptions);
        ...
        console.log(`✓ Converted: ${path.basename(ansiFile)} → ${path.basename(outputFile)}`);

...
Сгенерированное поведение при запуске определяется явными параметрами и сохраненной конфигурацией. `-AutoShow` явно включает отображение, а `-DefaultStartupScript` выбирает именованный сценарий. Сценарии Pokémon участвуют на общих основаниях; новые управляемые профили не задают вопросов о Pokémon и не добавляют `-IncludePokemon`. Если не используется `-SkipCacheBuild`, команда может предварительно разогревать записи кэша, выбранные политикой, после обновления профиля.
...
Использование устаревшего параметра совместимости в существующем вызове автоматизации:

```powershell
Add-ColorScriptProfile -IncludePokemon
```

Параметр принимается без сообщений и не выполняет действий в течение одного выпуска совместимости. Сценарии Pokémon уже участвуют на общих основаниях, а созданный профиль вызывает обычную команду `Show-ColorScript`.
...
Устаревший параметр совместимости. В течение одного выпуска он принимается без сообщений и не выполняет действий; цветовые сценарии Pokémon уже участвуют на общих основаниях, а созданные профили не добавляют этот параметр.
...
Устаревший параметр совместимости. В течение одного выпуска он принимается без сообщений и не выполняет действий, поскольку при создании профиля больше не задаются вопросы о Pokémon.
...
- **IncludePokemon** (bool): всегда `$true`; временно сохранено для совместимости объекта результата

...
ConfigurationRootCouldNotBeResolved = 'Корневой каталог конфигурации не удалось разрешить.'
UnableToResolveProfilePath = 'Не удалось разрешить путь к профилю '
FailedToExecuteColorscript = 'Не удалось выполнить colorscript '
FailedToBuildCacheForScript = 'Не удалось построить кэш для $($selection.Name).'
CacheBuildFailedForScript = 'Сборка кэша не удалась для $($selection.Name): $($cacheResult.StdErr.Trim())'
ScriptAlreadyExists = 'Скрипт ''$targetPath'' уже существует. Используйте -Force для перезаписи.'
ProfilePathNotDefinedForScope = 'Путь к профилю для области ''$Scope'' не определен.'
ScriptPathNotFound = 'Путь к скрипту не найден.'
...
UnableToRetrieveFileInfo = 'Не удалось получить информацию о файле для ''{0}'': {1}'
UnableToReadCacheInfo = 'Не удалось прочитать информацию о кэше для ''{0}'': {1}'

# Warning Messages
NoColorscriptsFoundMatchingCriteria = 'Colorscripts, соответствующие указанным критериям, не найдены.'
NoScriptsMatchedSpecifiedFilters = 'Ни один скрипт не соответствует указанным фильтрам.'
NoColorscriptsAvailableWithFilters = 'Colorscripts с указанными фильтрами недоступны.'
NoColorscriptsFoundInScriptsPath = 'Colorscripts не найдены в $script:ScriptsPath'
NoScriptsSelectedForCacheBuild = 'Скрипты для сборки кэша не выбраны.'
...
ProfileUpdatesNotSupportedInRemote = 'Обновления профиля не поддерживаются в удаленных сеансах.'
...
CacheBuildSummary = '`nСводка сборки кэша:'
...
MultipleColorscriptsMatched = 'Несколько colorscripts соответствуют предоставленным шаблонам имен: $($matchedNames -join '', ''). Отображение ''$($orderedMatches[0].Name)''.'
...
RemoteSessionDetected = 'Обнаружен удаленный сеанс.'

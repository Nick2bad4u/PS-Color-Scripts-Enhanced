...
- Review status: 2000-2026 review-complete, with every accepted work imported; 1990-1999 still in progress
...
The 2000-2026 review is complete and its accepted works are in the module. Across those 27 archive years, the audit covered 1,239 packs and 14,623 `.ANS` or `.ICE` candidates, accepted 3,674 works, and emitted 6,642 scripts after exact row or panel splitting. The latest completed tranche contributed 351 works as 393 scripts from 2000. The checked-in compact checkpoint records each year's inventory fingerprint, disposition totals, and accepted source/hash list.

The older archive is not complete. Subtracting the 1,195 candidates now reviewed for 2000 from the previous estimate leaves approximately 33,401 candidates across ten years from 1990 through 1999. That total is explicitly an estimate, not a permanent inventory count: archive refreshes, API corrections, and normalized duplicate detection can change it.

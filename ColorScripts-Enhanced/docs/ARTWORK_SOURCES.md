...
- Review status: 2001-2026 review-complete, with every accepted work imported; 1990-2000 still in progress
...
The 2001-2026 review is complete and its accepted works are in the module. Across those 26 archive years, the audit covered 1,044 packs and 13,428 `.ANS` or `.ICE` candidates, accepted 3,323 works, and emitted 6,249 scripts after exact row or panel splitting. The latest completed tranche contributed 310 works as 334 scripts from 2001. The checked-in compact checkpoint records each year's inventory fingerprint, disposition totals, and accepted source/hash list.

The older archive is not complete. Subtracting the 1,350 candidates now reviewed for 2001 from the previous estimate leaves approximately 34,596 candidates across 11 years from 1990 through 2000. That total is explicitly an estimate, not a permanent inventory count: archive refreshes, API corrections, and normalized duplicate detection can change it.
